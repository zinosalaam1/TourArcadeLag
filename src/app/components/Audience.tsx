      ...
      <div className="relative z-20 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {audiences.map((audience, index) => (
            ...
            >
              <div className={`relative h-full bg-gradient-to-br from-${audience.color}-900/40 to-black/60 backdrop-blur-xl border border-${audience.color}-500/30 rounded-3xl p-8 overflow-hidden transition-all duration-300`}>
                {/* Glow Effect */}
                <div className={`absolute inset-0 bg-gradient-to-br from-${audience.color}-600/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300`} />
                
                {/* Icon */}
                <div className="relative z-10 mb-6">
                  <div className={`bg-gradient-to-br from-${audience.color}-600/30 to-${audience.color}-900/30 backdrop-blur-xl p-4 rounded-2xl inline-block`}>
                    <audience.icon className={`w-12 h-12 text-${audience.color}-400`} />
                  </div>
                </div>

                {/* Counter with brighter colors */}
                <div className="relative z-10">
                  <p className={`text-6xl md:text-7xl mb-3 bg-gradient-to-r from-${audience.color}-300 to-${audience.color}-100 bg-clip-text text-transparent drop-shadow-lg`}>
                    <AnimatedCounter target={audience.count} suffix={audience.suffix} />
                  </p>
                  <p className="text-xl text-gray-200">{audience.label}</p>
                </div>

                {/* Decorative Corner */}
                <div className={`absolute -bottom-8 -right-8 w-32 h-32 bg-gradient-to-br from-${audience.color}-600/20 to-transparent rounded-full blur-2xl group-hover:scale-150 transition-transform duration-500`} />
              </div>
            </motion.div>
          ))}
        </div>
